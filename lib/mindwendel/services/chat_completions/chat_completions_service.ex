defmodule Mindwendel.Services.ChatCompletions.ChatCompletionsService do
  @callback generate_ideas(String.t(), list(map()), list(map()), String.t()) ::
              {:error, any()} | {:ok, any()}
  @callback classify_labels(String.t(), list(map()), list(map()), String.t()) ::
              {:error, any()} | {:ok, any()}
  ...
  def classify_labels(title, labels, ideas, locale \\ "en"),
    do: impl().classify_labels(title, labels, ideas, locale)
